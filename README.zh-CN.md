...
5. 资产计划：用户提供、本地已有、或 imagegen 生成的资产，并诚实说明缺失资产。如果 missing visual assets 会削弱页面，就在 Codex 里通过 imagegen 做 text-to-image 生成后再实现，除非该资产必须是真实或官方素材。
...
真实资产、本地资产和生成资产都会影响界面。可以使用生成资产，但必须明确它的角色，不能伪装成真实产品证明。当用户没有提供足够强的视觉媒体时，先 generate with imagegen，并按角色挑选资产，不要围绕空占位继续设计。
